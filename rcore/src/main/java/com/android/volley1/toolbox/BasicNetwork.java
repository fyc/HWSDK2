...
import com.android.volley1.Cache;
...
public class BasicNetwork implements Network {
    ...
    public NetworkResponse performRequest(Request<?> request) throws VolleyError {
         ...
         * 增加进度监听
         */
        mProgressListener = request.getProgressListener();
        ...
        while (true) {
            ...
            try {
                ...
                if ((statusCode < 200 || statusCode > 299) && statusCode != 401) {


                    if (statusCode >= 500 && statusCode < 600) {
                        RequestManager requestManager = RequestManager.getInstance(CoreManager.getContext());
                        requestManager.addRequestTime(request.getUniqueId());
                        requestManager.addFailRequests(request.getUniqueId(), statusCode);
                    }
                ...
                if (responseContents != null) {
                    ...
                    return new NetworkResponse(statusCode, responseHeaders,httpResponse.getEntity().getContent(), false,
                            SystemClock.elapsedRealtime() - requestStart);
    ...
    private byte[] entityToBytes(HttpEntity entity) throws IOException, ServerError {
        ...
        if (total > 20 *1024 *1024 ){
            VolleyLog.d("contentLength is so large so return stream %d ", total);
            return null;
        }

        if (mProgressListener != null)
            mProgressListener.onStart(total);

        PoolingByteArrayOutputStream bytes =
                new PoolingByteArrayOutputStream(mPool, (int) total);
        byte[] buffer = null;
        try {
            ...
            while ((count = in.read(buffer)) != -1) {
                ...
                curPos+=count;
                if (mProgressListener != null)
                    mProgressListener.onProgress(curPos,total);
            ...
            if (mProgressListener != null)
                mProgressListener.onFinish(total);
