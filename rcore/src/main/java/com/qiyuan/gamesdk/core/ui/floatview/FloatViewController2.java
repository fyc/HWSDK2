...
class FloatViewController2 implements IViewController {
...
//            imgFloatView.setVisibility(View.VISIBLE);
    ...
    public FloatViewController2(Context mContext) {
...
//            windowManagerParams.x = screenWidth - floatMoveViewWith;
            windowManagerParams.x = screenWidth - 25;//- 25而不- floatMoveViewWith是为了收起时贴着边缘
            windowManager.updateViewLayout(moveView, windowManagerParams); // 刷新显示
            windowManagerParams.x = screenWidth - floatMoveViewWith;//添加这一句时，是还原整个控件在屏幕中的位置，移动时不会出现不动的情况
...
//        imgFloatView.setImageResource(R.drawable.sdk_float_window_normal);
        imgFloatView.setVisibility(View.GONE);
        floatview_user_center.setVisibility(View.VISIBLE);
